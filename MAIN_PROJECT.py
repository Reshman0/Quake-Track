...
def main_menu():
    ...
    window.title("EARTHQUAKE REPORT SYSTEM")  

    title_label = tk.Label(window, text="EARTHQUAKE REPORT SYSTEM", font=("Arial", 16, "bold"))
    title_label.pack(pady=10)

    options_frame = tk.Frame(window)
    options_frame.pack(side=tk.TOP, anchor=tk.CENTER)

    admin_btn = tk.Button(options_frame, text="Admin Login", command=admin_menu, width=20, height=2)
    admin_btn.pack(pady=5)

    report_btn = tk.Button(options_frame, text="Make a Report", command=user_menu, width=20, height=2)
    report_btn.pack(pady=5)

    view_btn = tk.Button(options_frame, text="View Recent Earthquakes", command=recent_earthquakes, width=20, height=2)
    view_btn.pack(pady=5)

    button = tk.Button(options_frame, text="Exit", command=window.quit, width=10, height=2)
    button.pack(pady=5)
...
def user_menu():
    ...
    title_label = tk.Label(window, text="Select an option:", font=("Arial", 16, "bold"))
    title_label.pack(pady=10)

    options_frame = tk.Frame(window)
    options_frame.pack()

    button1 = tk.Button(options_frame, text="I am trapped under rubble", command=under_rubble, width=30, height=2)
    button1.pack(pady=5)

    button2 = tk.Button(options_frame, text="I will report a debris", command=reporting_debris, width=30, height=2)
    button2.pack(pady=5)

    button3 = tk.Button(options_frame, text="I need supplies", command=supplies, width=30, height=2)
    button3.pack(pady=5)
    
    button4 = tk.Button(options_frame, text="Return Main Menu", command=main_menu, width=30, height=2)
    button4.pack(pady=5)
...
def under_rubble():
    def submit_report():
        ...
        apartment_name = apartment_entry.get()
        health_condition = health_condition_combo.get()

        if apartment_name in pd.read_csv("DebrisReport.csv")["APARTMENT NAME"].tolist():
    ...
    submit_button = tk.Button(window, text="Submit", command=submit_report, width=15)
    submit_button.pack(pady=20)

    button1 = tk.Button(window, text="Return Upper Menu", command=user_menu, width=30, height=2)
    button1.pack(pady=5)
...
def reporting_debris():
    def submit_rprt():
        ...
        if apartment_name in (pd.read_csv("DebrisReport.csv").to_dict("list"))["APARTMENT NAME"]:
            result_label.config(text="A report has already been made for this apartment.")
        else:
    ...
    submit_button = tk.Button(window, text="Submit", command=submit_rprt, width=15)
    submit_button.pack(pady=20)
...
def supplies():
    ...
    supply_label = tk.Label(window, text="Supplies:", font=("Arial", 12))
    supply_label.pack(pady=10)

    supply_entry = tk.Entry(window, width=30)
    supply_entry.pack()

    add_button = tk.Button(window, text="Add Supply", command=add_supply, width=15)
    add_button.pack(pady=5)

    submit_button = tk.Button(window, text="Submit", command=submit_supply, width=15)
    submit_button.pack(pady=20)
...
def admin_menu():
    ...
    button1 = tk.Button(options_frame, text="See Information About Earthquake Reports", command=show_report_table, width=35, height=2)
    button1.pack(pady=5)

    button2 = tk.Button(options_frame, text="See Information About Supply Requests", command=show_supply_table, width=35, height=2)
    button2.pack(pady=5)

    button3 = tk.Button(options_frame, text="View General Data Reports", command=report, width=35, height=2)
    button3.pack(pady=5)

    button4 = tk.Button(options_frame, text="Return To Main Menu", command=main_menu, width=30, height=2)
    button4.pack(pady=5)
...
def report():
    def show_most_requested_supplies():
        ...
        fig.clear()
        plt.pie(supply_dict.values(), labels=supply_dict.keys(), autopct="%1.1f%%")
    ...
    label = tk.Label(window, text="Select a report option:")
    label.pack(pady=10)
    ...
    return_button = tk.Button(window, text="Return to Upper Menu", command=admin_menu, width=30, height=2, bg="red", fg="white")
...
def recent_earthquakes():
    ...
    button = tk.Button(window, text="Return To Main Menu", command=main_menu, width=30, height=2)
    button.pack(pady=5) 

    fetch_button = tk.Button(window, text="Get Data", command=fetch_earthquake_data, bg="#4caf50", fg="white", width=15, height=2)
...
def show_report_table():
    ...
    button = tk.Button(window, text="Return To Upper Menu", command=admin_menu, width=30, height=2)
    button.pack(pady=5) 
...
with open('DebrisReport.csv','w', newline='',encoding='utf-8') as file2:
    ...
    ["Kahramanmaras", "Elbistan", "(strtname)", "Gul", "Heavily Injured", "No Help Yet"],
    ["Hatay", "Iskenderun", "(strtname)", "Cennet", "Injured", "No Help Yet"],
    ["Gaziantep", "Karkamis", "(strtname)", "Nesil", "Heavily Injured", "No Help Yet"],
    ["Malatya", "Darende", "(strtname)", "Seref", "Injured", "No Help Yet"],
    ["Diyarbakir", "Sur", "(strtname)", "Zafer", "Injured", "No Help Yet"],
    ["Sanliurfa", "Harran", "(strtname)", "Dogus", "Injured", "No Help Yet"],
    ["Adiyaman", "Golbasi", "(strtname)", "Gunes", "No Injuries", "No Help Yet"],
    ["Osmaniye", "Kadirli", "(strtname)", "Sahin", "Heavily Injured", "No Help Yet"],
    ["Adana", "Ceyhan", "(strtname)", "Cinar", "Heavily Injured", "No Help Yet"],
    ["Malatya", "Arguvan", "(strtname)", "Hayat", "Injured", "No Help Yet"],
    ["Diyarbakir", "Bismil", "(strtname)", "Emek", "No Injuries", "No Help Yet"],
    ["Kilis", "Mazidagi", "(strtname)", "Huzur", "Injured", "No Help Yet"],
    ["Sanliurfa", "Harran", "(strtname)", "Gonul", "Heavily Injured", "No Help Yet"],
    ["Adiyaman", "Samsat", "(strtname)", "Gozde", "No Injuries", "No Help Yet"],
    ["Osmaniye", "Kadirli", "(strtname)", "Isik", "Heavily Injured", "No Help Yet"],
    ["Adana", "Kozan", "(strtname)", "Deniz", "Heavily Injured", "No Help Yet"],
    ["Kahramanmaras", "Pazarcik", "(strtname)", "Nazar", "Injured", "No Help Yet"],
    ["Hatay", "Dörtyol", "(strtname)", "Gundogan", "No Injuries", "No Help Yet"]]
...
with open('SupplyReport.csv','w', newline='',encoding='utf-8') as file1:
    ...
    ['Hatay', 'Dörtyol', 'Food'],
    ['Kahramanmaras', 'Göksun', 'Clothes'],
    ['Osmaniye', 'Kadirli', 'Water'],
    ['Adiyaman', 'Merkez', 'Tent'],
    ['Adana', 'Merkez', 'Medicine'],
    ['Gaziantep', 'Araban', 'Food'],
    ['Sanliurfa', 'Bozova', 'Food'],
    ['Diyarbakir', 'Bismil', 'Medicine'],
    ['Malatya', 'Yeşilyurt', 'Water'],
    ['Hatay', 'Iskenderun', 'Tent'],
    ['Kahramanmaras', 'Pazarcık', 'Tent'],
    ['Osmaniye', 'Düziçi', 'Water'],
    ['Adiyaman', 'Samsat', 'Food'],
